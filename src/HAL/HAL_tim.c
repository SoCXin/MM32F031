...
void TIM_TimeBaseInit(TIM_TypeDef *TIMx, TIM_TimeBaseInitTypeDef *TIM_TimeBaseInitStruct)
{
    ...
    tmpcr1 = (uint16_t)(TIMx->CR1);

    if ((TIMx == TIM1) || (TIMx == TIM2) || (TIMx == TIM3))
    {
        /* Select the Counter Mode */
        tmpcr1 &= (uint16_t)(~((uint16_t)(TIM_CR1_DIR | TIM_CR1_CMS)));
        tmpcr1 |= (uint16_t)TIM_TimeBaseInitStruct->TIM_CounterMode;
    }
    TIMx->CR1 = tmpcr1;
...
void TIM_OC1Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    tmpccer = (uint16_t)(TIMx->CCER);
    /* Get the TIMx CR2 register value */
    tmpcr2 =(uint16_t)( TIMx->CR2);

    /* Get the TIMx CCMR1 register value */
    tmpccmrx = (uint16_t)(TIMx->CCMR1);
...
void TIM_SelectOCxM(TIM_TypeDef *TIMx, uint16_t TIM_Channel, uint16_t TIM_OCMode)
{
    ...
    tmp1 = (uint16_t)((CCER_CCE_SET) << (uint16_t)TIM_Channel);
...
ITStatus TIM_GetITStatus(TIM_TypeDef *TIMx, uint32_t TIM_IT)
{
    ...
    itstatus = (uint16_t)(TIMx->SR & TIM_IT);

    itenable = (uint16_t)(TIMx->DIER & TIM_IT);
